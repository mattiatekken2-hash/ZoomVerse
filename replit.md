...
- **Official Production Overhaul**:
  - **Instant DB Sync**: Every balance change triggers immediate server sync (no debounce). Uses queue-based sync to avoid flooding: if a sync is in-flight, the next one queues and fires as soon as the current one completes. `beforeunload` uses `navigator.sendBeacon` for guaranteed delivery on page close. `visibilitychange` settles farming and syncs on resume. All server→client reconciliation uses `Math.max(local, server)` so the client never loses balance from stale server data.
