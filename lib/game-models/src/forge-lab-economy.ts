...
/**
 * Runtime farm bonus from the already-stored float (0–1).
 * Never writes inventories: callers derive yield/UI from this.
 * +0% at float 0, +12% at float 1. Pizza 3.5→3.92; House 5.6 stays above.
 */
...
/** Base rate × (1 + float × 12%). Null if not a Lab model. */
...
/**
 * Effective Lab farm for UI + settle. Uses stored float when present.
 * Never below the stored or canonical base (float only adds).
 */
