...
import {
  ...
  finishArcadeRun, buyArcadeLifeWithTon,
...
export function ArcadePage({ telegramId, visible }: ArcadePageProps) {
  ...
  const handleBuyLife = useCallback(async () => {
    ...
    setBusy(true);
    try {
      const r = await buyArcadeLifeWithTon(telegramId);
      if (r.ok) {
        setToast({ text: `+1 vita extra (-${r.costTon} TON)`, ok: true });
        // Notify rest of the app to re-sync TON balance epoch.
        window.dispatchEvent(new Event("zoom-data-refresh"));
      } else {
        setToast({ text: r.message || "Saldo TON insufficiente", ok: false });
      }
      await refresh();
    } finally {
      setBusy(false);
    }
  }, [telegramId, busy, refresh]);
  ...
  const handleGameEnd = useCallback(async (zEarned: number, durationMs: number) => {
    ...
    if (r.ok) {
      setToast({
        text: r.capped ? `+${r.credited} Z (TETTO 500!)` : `+${r.credited} Z`,
        ok: true,
      });
    }
    await refresh();
  ...
  const cost = state?.extraLifeCostTon ?? 0.10;
  void tick;
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-6" style={{ minHeight: 0, WebkitOverflowScrolling: "touch" }}>
        ...
        <div className="rounded-2xl p-4 border mb-3 relative overflow-hidden"
          ...
          <div className="text-xs mt-1" style={{ color: "rgba(255,255,255,0.35)" }}>
            Convertibili in $ZOOM (presto disponibile)
          </div>
          <button
            disabled
        ...
        <div className="rounded-2xl p-4 border mb-3" style={{ borderColor: "rgba(0,242,254,0.16)", background: "rgba(0,242,254,0.03)" }}>
          <div className="grid grid-cols-2 gap-2 mb-3">
            <div className="rounded-xl p-3 border" style={{ borderColor: "rgba(0,230,118,0.2)", background: "rgba(0,230,118,0.05)" }}>
              <div className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "rgba(0,230,118,0.7)" }}>Vita gratis</div>
              <div className="font-black text-base mt-1" style={{ color: freeReady ? "#00e676" : "rgba(255,255,255,0.4)" }}>
                {freeReady ? "DISPONIBILE" : formatCountdown(state?.nextFreeLifeMs ? state.nextFreeLifeMs - (tick * 1000) : 0)}
              </div>
            </div>
            <div className="rounded-xl p-3 border" style={{ borderColor: "rgba(196,113,237,0.2)", background: "rgba(196,113,237,0.05)" }}>
              <div className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "rgba(196,113,237,0.7)" }}>Vite extra</div>
          ...
          >
            {busy ? "…" : canPlay ? "▶ START — ZOOM JUMP" : "Nessuna vita disponibile"}
          </button>

          <button
            onClick={handleBuyLife}
            disabled={busy || !telegramId || tonBalance < cost}
            className="w-full mt-2 py-2.5 rounded-xl font-black text-xs tracking-widest uppercase border"
            style={{
              background: tonBalance >= cost ? "rgba(0,210,255,0.08)" : "rgba(255,255,255,0.02)",
              color: tonBalance >= cost ? "#00d2ff" : "rgba(255,255,255,0.25)",
              borderColor: tonBalance >= cost ? "rgba(0,210,255,0.25)" : "rgba(255,255,255,0.06)",
              cursor: tonBalance >= cost && !busy ? "pointer" : "not-allowed",
            }}
            data-testid="btn-buy-extra-life"
          >
            + Vita extra · {cost} TON {tonBalance < cost ? `(saldo: ${tonBalance.toFixed(3)})` : ""}
          </button>

          <div className="text-[10px] mt-2 leading-relaxed text-center" style={{ color: "rgba(255,255,255,0.3)" }}>
            1 vita gratis ogni 24h · tetto 500 Z per partita
          </div>
        </div>
              ...
              return (
                <div key={e.telegramId} className="rounded-xl border flex items-center gap-3 px-3 py-2"
                  ...
                  <div className="flex-1 font-bold text-sm truncate" style={{ color: isUser ? "#ffd700" : "rgba(255,255,255,0.6)" }}>
                    {e.firstName}{isUser && <span className="text-xs opacity-50 ml-1">(tu)</span>}
                  </div>
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-4 left-0 right-0 text-center text-xs" style={{ color: "rgba(255,255,255,0.45)" }}>
        Tocca e trascina per muovere · Salto automatico
      </div>
    </div>
  );
}
