...
const SaveBody = z.object({
  ...
  claimedBonusNova: z.number().int().min(0).optional(),
  ...
  try {
    const rows = await db
      .select({
        ...
        claimedBonusNova: usersTable.claimedBonusNova,
    ...
    res.json({
      ...
      claimedBonusNova: row.claimedBonusNova ?? 0,
      ...
      if (lostCount >= SHRINK_GUARD_THRESHOLD) {
        ...
        await tx
          ...
          .set({
            ...
            ...(claimedBonusNova  != null ? { claimedBonusNova:  sql`GREATEST(${usersTable.claimedBonusNova},  ${claimedBonusNova})`  } : {}),
